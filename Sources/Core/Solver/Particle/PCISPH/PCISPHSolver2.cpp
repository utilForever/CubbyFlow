...
namespace
{
struct PressureUpdateData2
{
    const Array1<Vector2D>& positions;
    const Array1<Array1<size_t>>& neighborLists;
    const SPHStdKernel2& kernel;
    double mass;
...
void UpdatePressureFromDensityError(size_t i, const PressureUpdateData2& data)
{
...
void PCISPHSolver2::AccumulatePressureForce(double timeIntervalInSeconds)
{
    ...
    for (unsigned int k = 0; k < m_maxNumberOfIterations; ++k)
    {
        ...
        const PressureUpdateData2 update{ m_tempPositions,
...
{
    return std::make_shared<PCISPHSolver2>(m_targetDensity, m_targetSpacing,
                                           m_relativeKernelRadius);
}
}  // namespace CubbyFlow
